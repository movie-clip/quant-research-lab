...
describe('DiagnosticsPanel', () => {
  it('renders the diagnostics quant shell and behavior-through-time section only', () => {
    ...
    expect(screen.getByText('Decision cards stay compact: every value is mapped directly from backend summary outputs.')).toBeTruthy()
    expect(screen.getByText('Start with source truth and availability, then scan the compact readout before moving into the deeper diagnostics.')).toBeTruthy()
    expect(screen.getByText('Use the selected window to review risk path, benchmark-relative behavior, and factor loadings without inventing unsupported history.')).toBeTruthy()
    expect(screen.getByText(/Status: Synthetic snapshot-history \/ Historical sections live \/ Window 60d partial\./)).toBeTruthy()
