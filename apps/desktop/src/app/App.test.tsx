...
describe('App', () => {
  ...
  it('displays and saves admission review metadata for a variant against the nearest imported source anchor', async () => {
    const anchorAdmissionSummary = {
      ...bootstrapPayload.admission_summary,
      checks: bootstrapPayload.admission_summary.checks.map((check) => check.check_id === 'residual_cash_comparability'
        ? { ...check, message: 'Anchored imported snapshot cash evidence requires local review.' }
        : check),
    }
    const backendAdmissionSummary = {
      ...bootstrapPayload.admission_summary,
      checks: bootstrapPayload.admission_summary.checks.map((check) => check.check_id === 'residual_cash_comparability'
        ? { ...check, message: 'Backend variant cash evidence should not render.' }
        : check),
    }
    const baseNode = {
      id: 'node-1',
    ...
    const importedSnapshotNode: PortfolioNode = {
      ...
      changeSummary: { label: 'IB 2026', changedPositionsCount: 22, changedSectorsCount: 10, grossExposureDelta: 50368.17, netCapitalDelta: 50368.17 },
      portfolioSnapshot: persistedSnapshot,
      source: buildImportedSource({ importedFileNames: ib2026LoadedFiles, importedAt: '2026-04-14T00:00:00Z', importer: 'interactive_brokers', baseCurrency: 'USD', historyContext: ib2026HistoryContext, importedHistorySnapshot: ib2026BootstrapPayload.snapshot, admissionSummary: anchorAdmissionSummary }),
    }
    const variantNode = {
      id: 'node-3',
      workspaceId: 'workspace-1',
    ...
    vi.spyOn(portfolioWorkspaceStorage, 'getLastOpenedWorkspaceState').mockResolvedValue({ workspaceId: 'workspace-1', activeNodeId: 'node-3', activeDraftId: null, selectedExposureSnapshotId: 'node-3', lastOpenedAt: '2026-04-14T00:10:00Z' })
    vi.spyOn(portfolioWorkspaceStorage, 'getWorkspace').mockResolvedValue({
      id: 'workspace-1',
      name: 'Portfolio Workspace',
      createdAt: '2026-04-10T00:00:00Z',
    ...
    vi.spyOn(portfolioWorkspaceStorage, 'getWorkspaceNodes').mockResolvedValue([baseNode, importedSnapshotNode, variantNode])
    vi.spyOn(portfolioWorkspaceStorage, 'getNode').mockImplementation(async (nodeId: string) => {
      if (nodeId === 'node-3') return variantNode
      if (nodeId === 'node-2') return importedSnapshotNode
      return baseNode
    })
    vi.spyOn(portfolioWorkspaceStorage, 'getDraft').mockResolvedValue(null)
    const saveDispositionSpy = vi.spyOn(portfolioWorkspaceStorage, 'saveImportAdmissionReviewDisposition').mockResolvedValue({
      workspace: {
        id: 'workspace-1',
        name: 'Portfolio Workspace',
        createdAt: '2026-04-10T00:00:00Z',
        ...
        rootNodeId: 'node-1',
        activeNodeId: 'node-3',
        source: buildImportedSource({ importedFileNames: ['IB2025.pdf'], importedAt: '2026-04-10T00:00:00Z', importer: 'interactive_brokers', baseCurrency: 'USD' }),
      },
      node: importedSnapshotNode,
    })
    ...
    installFetchMock(async (input, init) => {
      ...
      if (pathname === '/api/engines/exposure/run' && method === 'POST') return jsonResponse(exposurePayload)
      if (pathname === '/api/engines/diagnostics/run' && method === 'POST') return jsonResponse(diagnosticsPayload)
      if (pathname === '/api/engines/dashboard-history/run' && method === 'POST') return jsonResponse({ ...dashboardHistoryPayload, admission_summary: backendAdmissionSummary })
      throw new Error(`Unhandled fetch: ${method} ${pathname}`)
    })

    render(<App />)

    await waitFor(() => expect(screen.getByText('Anchored imported snapshot cash evidence requires local review.')).toBeTruthy())
    expect(screen.queryByText('Backend variant cash evidence should not render.')).toBeNull()

    fireEvent.click(screen.getAllByRole('button', { name: 'Review exception' })[0])
    fireEvent.change(screen.getByLabelText('Residual cash review rationale'), { target: { value: 'Variant dashboard review stays anchored to imported snapshot source.' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save review metadata' }))

    await waitFor(() => expect(saveDispositionSpy).toHaveBeenCalledTimes(1))
    expect(saveDispositionSpy).toHaveBeenCalledWith(expect.objectContaining({
      workspaceId: 'workspace-1',
      nodeId: 'node-2',
      disposition: expect.objectContaining({
        check_id: 'residual_cash_comparability',
        rationale: 'Variant dashboard review stays anchored to imported snapshot source.',
        evidence_summary: expect.objectContaining({
          message: 'Anchored imported snapshot cash evidence requires local review.',
        }),
      }),
    }))
  })
