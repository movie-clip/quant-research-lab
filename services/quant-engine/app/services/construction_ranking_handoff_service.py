...
def preflight_generic_ranking_artifact_for_construction(
    artifact_id: str,
    *,
    store: GenericRankingArtifactStore | None = None,
) -> ConstructionRankingArtifactPreflightResponse:
    artifact = load_generic_ranking_artifact(artifact_id, store=store)
    if artifact.artifact_id != artifact_id:
        raise ValueError("ranking artifact handoff artifact_id does not match persisted artifact")
    return build_construction_preflight_response_from_generic_ranking_artifact(artifact)
