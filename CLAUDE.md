...
- **Commit gate hook** — enforced at two layers, both checking the same thing (`.claude/.last-test-pass` exists and is fresher than every changed non-`.md` file; the marker is written only by a fully green `run_all_tests.py` run). The **git-level** `scripts/githooks/pre-commit` (wired via `git config core.hooksPath scripts/githooks`, calling `scripts/hooks/git_pre_commit.py`) is the actual enforcement boundary — it fires on every `git commit` regardless of which tool or terminal invoked git. The **Claude Code** `scripts/hooks/pre_commit_gate.py` (PreToolUse, wired in `.claude/settings.json`, matched on the Bash tool) is a faster-feedback duplicate inside agent sessions, not the boundary itself — a commit issued through a different tool used to walk straight past it (fixed 2026-08-20; see `docs/product/review-2026-08-20-findings.md` F-R1). If a commit is blocked, re-run the suite — do not try to bypass either hook.
