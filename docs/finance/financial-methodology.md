...
- per-symbol sector metadata reaches construction through `generic_ranking` artifacts: the universe resolver retains a `symbol -> GICS sector` map (`UniverseSpecSnapshot.member_sectors`) for screen and `index_constituent` universes, the ranking engine stamps it onto each `GenericRankingRow.sector`, and the construction handoff threads it into the ranked-candidate contract. `etf_peer_group` / `custom_list` universes consult no sector source, so their rows carry no sector and `max_sector_weight` stays `not_evaluated`. `etf_ranking` and `intent_bound_etf_replacement_ranking` artifact handoffs do not yet carry sector metadata (sector ETFs are themselves sectors), so those handoffs also evaluate the cap as `not_evaluated`
