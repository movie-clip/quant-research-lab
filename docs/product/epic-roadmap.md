...
**Completed. Opened 2026-09-02. 4 stories authored (US-43.1–US-43.4); US-43.1
and US-43.2 Done 2026-09-02, US-43.3 Done 2026-09-03, US-43.4 Done 2026-09-05.**
...
- **US-43.4** — `import_engine_composer.py` (36 lines) is a pass-through. → fold
  into `import_engine.py`. **Shipped 2026-09-05** — body moved verbatim into
  `import_engine.py` as module-private `_compose_import_bootstrap_response`;
  sole call site retargeted; `import_engine_composer.py` deleted; goldens
  byte-identical.
...
| [US-43.4](../stories/US-43.4-collapse-import-engine-composer.md) | Collapse `import_engine_composer` into `import_engine` | Done |
...
| 2026-09-05 | US-43.4 | Behaviour-neutral fold of `services/import_engine_composer.py` (36 lines, one pass-through function) into `services/import_engine.py` as module-private `_compose_import_bootstrap_response` — body moved verbatim, including the delegated `build_import_admission_summary(snapshot)` call; the sole call site inside `build_import_bootstrap_from_snapshot` retargeted; three new imports added and the `app.schemas.reconciliation` import merged onto one line rather than duplicated. All three public entry functions (`build_import_bootstrap`, `build_import_bootstrap_from_portfolio_snapshot_request`, `build_import_bootstrap_from_snapshot`) kept their exact names and signatures. Quant-audit skipped by human-approved ruling (pure Pydantic-construction relocation, no `analytics/` or trust-classification code touched, `02-technical-plan.md` § Quant-audit ruling). `services/import_engine_composer.py` deleted via the human `git rm` step between the backend and test dispatches (no lane holds a delete tool); a new pin test (`test_import_engine_composer_module_is_gone`) asserts `ModuleNotFoundError` on import, proving the module is genuinely gone. `ImportedBootstrapResponse` schema untouched — no contract-shape change; one dangling code pointer corrected in `docs/contracts/exposure-fields.md`. No schema, route, `analytics/`, or `apps/desktop/src/**` change; `dashboardGoldens.ts` diff empty; backend goldens byte-identical. Integration gate PASS (independently re-ran backend suite + dead-code + `tsc` + goldens-drift checks, confirmed no dangling reference to the old module/function anywhere under `app/`). Tests: backend 985 passed (was 984 at US-43.3 close; +1 new pin test), dead-code gate (ruff/vulture/knip) clean, `tsc --noEmit` clean. Epic 43 — Engine Seam Consolidation is now Completed (all four stories Done). |
...
## Completed Epic: Epic 26 — Currency Exposure & Risk
...
| Date | Story | What shipped |
|---|---|---|
| 2026-08-11 | US-26.2 | **The consequence half of Epic 26 — and the first story this project shipped from a research brief that had previously been marked "do not build."** New `analytics/currency_risk.py` + `services/currency_risk_engine.py` + `POST /engines/currency-risk/run` + a self-fetching Currency Risk Contribution card (60d/252d, `Synthetic` badge). Decomposes each non-base holding's base-currency return into **local / FX / interaction** legs — an **exact identity**, `(1+r_base) = (1+r_local)(1+r_fx)`, not the approximation the old methodology text claimed — and splits portfolio variance by **component covariance**, `Var(r_p) = Cov(L,r_p) + Cov(F,r_p) + Cov(X,r_p)`, so the three shares sum to exactly 1.0 with no residual (the naive `Var(F)/Var(r_p)` was rejected: it ignores `Cov(L,F)` and does not account for the portfolio). **Measured live on the committed portfolio:** securities **96.70%**, currency **3.31%**, interaction **−0.013%**, sum **exactly 1.0**; per-currency EUR 0.0243 + GBP 0.0089 reconciles to the currency share; standalone vols 18.76% / 2.51%, correlation 0.127. So the researcher's 21% non-USD exposure drives ~3.3% of return variance — real but small next to the securities. The interaction share came out genuinely **negative** and renders unclamped, exercising the no-clamping rule on real data rather than only in a constructed test. **Two corrections during build:** (1) the request shape moved from the flat `PortfolioEngineRequest` to the **snapshot-wrapped** shape the sibling Exposure engines use — base-currency weights need the statement's own `fx_rates`, which the flat shape only materialises conditionally; (2) the brief suggested attaching this to the exposure engine, which fetches **no** history — doing so would have mixed snapshot analytics with synthetic history in one response (guardrail #2) and slowed every Exposure request, so it uses the established dedicated-route + self-fetching-card pattern instead. The `test_all_engine_run_routes_are_covered` guard caught the new route and it was added to `ENGINE_ROUTES` rather than waived. Highest-risk detail guarded two-directionally: local returns come from the **registry fund currency** (US-31.5), so a holding listed EUR but quoting USD gets a **zero** FX leg — a wrong assignment here is invisible to the identity check, since both legs would come from the same wrong split. +29 tests (11 analytics, 8 engine, 3 route, 7 card) + the panel-wiring pin. `dashboardGoldens.ts` byte-identical. 680 backend + 310 frontend green; tsc + dead-code gate clean. |
| 2026-08-11 | US-26.1 (follow-up) | **Ran the app and found the card did not render — in work that had just shipped green.** `composeExposureView` builds the Exposure view model **field by field**, and `currency_exposure` was not forwarded, so the backend computed it correctly and the adapter dropped it before the tab ever saw it. The card would never have appeared in the running app. Exactly the **Epic 25 failure mode** (a whole Dashboard surface silently disappeared while the backend stayed green and golden-pinned) and the **US-24.11** one (five disclosures computed and rendered nowhere). **Why the suite missed it:** `CurrencyExposureCard.test.tsx` feeds the component a fixture directly, so it can never exercise the adapter — and the `ExposurePanel.test.tsx` test the story's own test plan promised was the one test not written. Verified live first at the API seam (`POST /engines/exposure/run` returns EUR 8000×1.1657 = 9,325.60, GBP 2700×1.3444 = 3,629.88, non-base 52.85%, both disclosed as static-rate), which isolated the fault to the frontend. **Fixed:** adapter forwards the field; `currency_exposure` added to the shared exposure fixture so **ExposurePanel tests now run through `composeExposureView`** — the real seam — with a positive and a negative pin. Also deduplicated UI found while fixing: the card repeated the tab-level "No FX rate available for X" sentence verbatim; its note now states what unconverted values mean for *this card's* numbers instead, and a test pins that it does not repeat the panel's sentence. **The general lesson, worth keeping:** a component test with a hand-built fixture proves the component works, never that anything reaches it. Any new response field needs one test that travels the adapter. +3 frontend tests. 652 backend + 303 frontend green; tsc + dead-code gate clean; goldens byte-identical. |
...
| US-24.7 | Name the unnamed tolerances + delete the orphaned risk path | Done |
...
**Closed 2026-08-09** — all eleven stories done. The epic began as a
catalogue of hardcodes and magic numbers from Epic 23, and the discipline it
adopted — *re-validate every register row against the code before refactoring
it* — repeatedly turned "tidy this up" rows into real defects:

| Story | What the row said | What re-validation found |
|---|---|---|
| US-24.4 | "Freedom24 ISIN data gap" | A misread; the real gap was fail-safe parsing |
| US-24.6 | 3 config items | 2 real, 1 correct-as-is (`limit=500`) |
| US-24.9 | Med accuracy refinement | Correct — plus 2 fabrications the tripwire caught |
| US-24.10 | (created by US-24.9) | A live −7.90% / +9.61% fabrication on the Dashboard |
| US-24.5 | Med coupling debt | **High** defect: every Freedom24 trade and ESPP contribution unclassified |
| US-24.7 | Low-severity hardcodes | A methodology doc citing a code path that never ran |

Four of the eleven stories ended by logging a *new* finding rather than only
closing the old one (US-24.6 → renumbering fix, US-24.9 → US-24.10,
US-24.10 → US-24.11, US-24.5/24.7 → scoping corrections). The register is now
clear of high- and medium-severity rows.
...
| Date | Story | What shipped |
|---|---|---|
| 2026-08-09 | US-24.7 | **Epic 24's tail — and it contained a guardrail violation, not just hardcodes.** Re-validated item by item. **F-1 (the real finding):** the public `build_position_risk_contributions` had **no production caller** (the live Risk Summary path is the private `_build_position_risk_contributions`), yet `financial-methodology.md` named it as the source of per-position beta/correlation — so the canonical methodology document pointed a reader at a function that never executed. That is guardrail #1 ("every UI metric maps to one engine formula and one code path") failing in the doc rather than the code. Deleted it along with the `PortfolioRiskContribution` model only it used (on no response schema) and the three tests that existed solely for it; the US-30.5b observation-floor coverage is retained against the path that actually runs. **Why the dead-code gate missed it:** vulture counts test callers, so a function reachable only from tests is invisible to it — a general limitation worth remembering, not a gate bug. **F-2:** two unnamed financial tolerances became documented constants — `STATEMENT_RECONCILIATION_TOLERANCE` (0.25, decides whether a statement reconciliation check passes) and `PORTFOLIO_PROOF_TERMINAL_MATCH_TOLERANCE` (0.01), whose rationale states **why** it is 100× tighter than `REPLAY_RECONCILIATION_TOLERANCE`: it compares the proof path's own recomputation against the statement, where a cent of disagreement means the recomputation is wrong, while the replay tolerance absorbs genuine valuation residuals across a window. Values unchanged. **Three scoping corrections, recorded rather than built:** the `statement_importer` `/100` is a percentage→fraction unit conversion (self-evident, naming it would obscure it); the `"USD"` repetition (76 non-test sites) mixes base-currency *fallbacks* with currency *data*, so a blanket constant would conflate them and touch 76 sites for zero behaviour change; de-export is stale since US-23.8 shipped knip with `ignoreExportsUsedInFile`. Behaviour-neutral — `dashboardGoldens.ts` byte-identical, dead-code gate green after the deletion. +2 tests, −3 removed with the function. 643 backend + 294 frontend green; tsc clean. **Epic 24 closed.** |
