...
| US-30.5c | **Fix F-10 — done.** Scoping correction: "market-value chain everywhere" is unsafe on the imported ledger-replay path (a BUY fabricates a return — the F-1 class; reproduced 37.23% on IB2026's 2026-06-19). Resolved by a **provenance-selected** basis: the *synthetic* surfaces (rolling correlation/beta, factor attribution, factor model, stress, multi-benchmark) move onto the cash-excluded market-value chain; the imported ledger path keeps the trade-safe TWR. §Rolling Pearson Correlation rewritten to match §Indexed Return Series. Ledger-path cash de-dilution deferred → tech-debt US-24.9. Goldens byte-identical (imported path unchanged). | Medium |
