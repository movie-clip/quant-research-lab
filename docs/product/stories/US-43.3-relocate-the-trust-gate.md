...
- [ ] AC1 — A new module `services/trust_gate.py` holds, moved verbatim: both
...
- [ ] AC2 — `_has_any_symbol_price_history` is defined **once** in
...
- [ ] AC3 — `dashboard_history_engine.py` and `diagnostics_engine.py` import
...
- [ ] AC5 — No behavioural merge: the two `SectionTrust` builders remain
...
- [ ] AC6 — The US-23.8 dead-code gate stays green (`ruff` / `vulture` / `knip`
...
- [ ] T-43.3.1 — **Create `services/trust_gate.py` + merge the one identical
...
- [ ] T-43.3.2 — **Move the section-trust builders + output-admission gates.**
  ...
  Suite green.
- [ ] T-43.3.3 — **Move the dashboard return-basis classification helpers.**
  `classify_portfolio_return_basis`, `build_dashboard_return_basis_contract`,
  `build_dashboard_return_basis_evidence`. Wire `dashboard_history_engine`.
  Suite + dead-code gate green.
- [ ] T-43.3.4 — **Docs close-out.** Note `services/trust_gate.py` in
