...
| [US-34.9](US-34.9-source-adjusted-closes-for-verified-benchmark.md) | Source adjusted closes so the verified benchmark rung can fire | F-9, found by US-34.5: `_validate_verified_benchmark_slice` needs every row to carry `adjClose` **and** the fetch to come from `historical-price-eod/light` — but that endpoint returns none, so the two conditions cannot both hold and the rung is unreachable by construction. Its tests passed only on hand-written rows. Points the benchmark-only fetch seam at FMP's `historical-price-eod/dividend-adjusted`, keeps portfolio valuation on real traded prices, and pins the committed capture's actual `adjClose` coverage so code and data can never diverge silently again. Behaviour-neutral until the owner re-captures with an `FMP_API_KEY` | In progress — code landed; T-34.9.6 (re-capture) needs the owner's `FMP_API_KEY` |
...
Recommended order: US-34.2, US-34.6, US-34.3, US-34.4, US-34.8, US-34.7 and US-34.5 (done); US-34.9's code has landed; its last ticket is the owner-run FMP re-capture.
