...
| US-34.9 | Source adjusted closes so the verified benchmark rung can fire | F-9, found by US-34.5: `_validate_verified_benchmark_slice` requires every row to carry `adjClose` **and** the fetch to come from `historical-price-eod/light` — but the FMP client has only that one history method and no adjClose handling at all, so the two conditions cannot both hold. The verified benchmark path is unreachable in production, and its tests pass only on hand-written rows. Needs a new endpoint + a market-data re-capture | Next phase |
...
Recommended order: US-34.2, US-34.6, US-34.3, US-34.4, US-34.8 and US-34.7 (done); US-34.5 blocked on F-10; US-34.9 needs an FMP re-capture.
