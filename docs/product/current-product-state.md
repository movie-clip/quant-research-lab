...
- desktop policy authority is now stronger, but the visible ranking-to-construction bridge remains intentionally parameter-light: policy identity is backend-authoritative while `top_n` stays fixed at `2` and is now explicit in backend discovery plus handoff-path validation, only `max_position_weight` plus optional `min_position_weight`, optional `max_turnover_weight`, and optional `max_trade_intent_count` are desktop-editable, no new policies or ranking families are exposed, and some ranking-entry portfolio lineage assumptions remain narrow review-only defaults
