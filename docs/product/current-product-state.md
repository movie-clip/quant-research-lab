...
- `GET /strategy-lab/etf-ranking/artifacts/recent` exposes newest-first recent artifact discovery with optional `effective_peer_group` filtering — kept as a backend route for any external consumers, but the desktop ETF Ranking tab now reads recent artifacts through the generalized `/strategy-lab/ranking-artifacts/recent?artifact_kind=etf_ranking` path instead
- `GET /strategy-lab/etf-ranking/artifacts/recent/metadata` exposes discovered recent-run filter metadata for current consumers — kept as a backend route, but the desktop no longer calls it; peer-group filter options are now derived client-side from the unfiltered generalized recent response by deduplicating `effective_peer_group` values
...
- desktop `Review In Construction` now consumes authoritative backend `GET /construction/policies` discovery for policy identity on the shipped ranking-to-construction bridge; desktop no longer silently falls back to a hardcoded construction policy id, derives launch compatibility/default status from backend `launch_profile` metadata, auto-selects `top_n_equal_weight_v1` only when discovery truthfully marks it as the single `default`, surfaces only launch-compatible `top_n_equal_weight_v1` and explicit opt-in `top_n_linear_rank_weight_v1`, requires the catalog to truthfully advertise a `launch_top_n` value in the supported range `[2, 20]` in both row metadata and `launch_profile`, keeps `top_n_inverse_rank_weight_v1` excluded from the desktop launch profile, and otherwise fails closed before construction launch
- launch `top_n` is now configurable per request in the supported range `[2, 20]` (was fixed at 2). Per-policy catalog metadata still declares `launch_top_n = 2` as the recommended default; the value is independent runtime input. Backend validates `2 <= policy.top_n <= 20` at the handoff boundary
- the shipped artifact-backed ranking-to-construction launch browsers in Workspace (ETF, replacement, generic) now expose a required `Top N` integer field (default `2`, range `[2, 20]`), a required decimal `Max Position Weight` field with a shipped default of `0.60`, plus optional `Min Position Weight`, `Max Turnover Weight`, and `Max Trade Intent Count`; desktop validates `2 <= top_n <= 20` and integer, `0.5 <= max_position_weight <= 1`, treats blank optional fields as omitted, requires `0 < min_position_weight <= 0.5` and `min_position_weight <= max_position_weight` when populated, requires `0 <= max_turnover_weight <= 1` when populated, requires integer `max_trade_intent_count >= 0` when populated, and preserves the same narrow review-only handoff request shape
- the legacy ETF Ranking tab (`features/strategy-lab/EtfRankingPanel.tsx`) still launches construction with `top_n = 2` by default; surfacing the configurable Top N control on that surface is a deferred follow-up
- ETF Ranking and Workspace persisted ranking construction launch surfaces now also show compact readback text for the selected launch policy, whether it is the shipped default or opt-in path, the supported `top_n` range, required `max_position_weight`, and optional `min_position_weight`, `max_turnover_weight`, and `max_trade_intent_count`
...
- supported universe kinds: `etf_peer_group`, `custom_list`, `broad_equity_screen`/`sector_screen` (FMP `/stock-screener`), `index_constituent` with two `index_id` values: `sp500` (live FMP `/stable/sp500-constituent`) and `russell1000` (static JSON snapshot at `data/universe/index_snapshots/russell1000.json`, sourced from iShares IWB ETF holdings; bundled snapshot is a representative sample of large caps with explicit provenance metadata, not the full ~1000 names — full ingestion is deferred)
...
- desktop policy authority is now stronger, but the visible ranking-to-construction bridge remains intentionally parameter-light: policy identity and launch compatibility/default status are backend-authoritative; `top_n` is now configurable per request in `[2, 20]` (default `2`); desktop launch pickers surface only `top_n_equal_weight_v1` plus explicit opt-in `top_n_linear_rank_weight_v1`; only `top_n`, `max_position_weight`, plus optional `min_position_weight`, `max_turnover_weight`, and `max_trade_intent_count` are desktop-editable; no new ranking families are exposed; and some ranking-entry portfolio lineage assumptions remain narrow review-only defaults
