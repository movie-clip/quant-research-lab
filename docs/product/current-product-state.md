...
- desktop `Review In Construction` now consumes authoritative backend `GET /construction/policies` discovery for policy identity on the shipped ranking-to-construction bridge; desktop no longer silently falls back to a hardcoded construction policy id, auto-selects `top_n_equal_weight_v1` only when backend discovery actually returns it, requires the catalog to truthfully advertise the fixed `launch_top_n = 2` boundary, and otherwise fails closed before construction launch
- desktop ranking-to-construction entry points now expose a required decimal `Max Position Weight` field with a shipped default of `0.60` plus an optional `Min Position Weight`; desktop validates `0.5 <= max_position_weight <= 1`, treats blank `min_position_weight` as omitted, and if populated requires `0 < min_position_weight <= 0.5` and `min_position_weight <= max_position_weight`, while keeping `top_n = 2`, omitting `max_turnover_weight` and `max_trade_intent_count`, and preserving the narrow review-only handoff request shape across all three desktop entry points
...
- desktop policy authority is now stronger, but the visible ranking-to-construction bridge remains intentionally parameter-light: policy identity is backend-authoritative while `top_n` stays fixed at `2` and is now explicit in backend discovery plus handoff-path validation, only `max_position_weight` plus optional `min_position_weight` are desktop-editable, other hard-constraint knobs stay hidden, and some ranking-entry portfolio lineage assumptions remain narrow review-only defaults
...
- monitoring persistence is still narrow: only canonical monitor-definition artifacts, one canonical latest observation artifact per definition, one canonical latest-evaluation snapshot sidecar per definition, and append-only canonical evaluation-history entries for `benchmark_trend_overlay_v1` are shipped, list remains the narrow artifact inventory view, catalog/recent discovery is read-only and additive on top of the create/get/evaluate surface, discovery reads latest observation state only from the canonical persisted observation artifact and never reconstructs it from latest-snapshot sidecars or history, evaluation-history inspection is read-only and newest-first only, the additive definition-scoped alert-review timeline is also read-only and newest-first only while sourcing rows only from the canonical persisted latest observation artifact plus append-only history entries, the additive recovered alert review queue is discovery-only and newest-first only while sourcing rows only from canonical persisted latest observation plus latest snapshot plus prior alert-history lineage, requiring latest observation/latest snapshot/latest persisted history alignment to an informational recovered state before reopening the definition-scoped timeline latest-observation event, history reads only append-only persisted entries, discovery filters remain limited to low-cardinality persisted metadata including latest-observation presence/status/classification/cause-code/recency and latest-snapshot cause-code, latest-observation metadata is surfaced only when a canonical persisted observation artifact is present and valid, latest-evaluation snapshot metadata remains additive, recency is derived strictly from persisted `evaluated_at`, successful evaluation overwrites the canonical observation and latest-snapshot sidecars while appending one canonical persisted history entry, `cause_code` remains explicit `null` for `ok` and `threshold_breach`, degraded and unavailable outcomes persist authoritative canonical non-null cause codes, legacy compatibility remains load-only and does not hydrate missing cause codes on old persisted observation/snapshot/history artifacts, and evaluation is review-only with explicit `ok` / `threshold_breach` / `degraded` / `unavailable` outcomes
...
- broader persisted monitoring observations, alerts, and review history beyond the current narrow `benchmark_trend_overlay_v1` observation/evaluation seam
