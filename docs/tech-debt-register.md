...
| US-24.11 — replay disclosures are computed but never rendered | Med — **NEW (logged by US-24.10, 2026-08-09)** | All **five** dashboard-history replay disclosures — `fx_fallback_currencies` (US-27.8), `unpriced_replay_symbols` (US-31.2), `replay_cash_anchor` (US-31.3), `withheld_return_dates` (US-31.3) and `trade_price_anchored_symbols` (US-24.10) — are computed, schema'd, contract-documented and golden-pinned, but `DashboardPanel.tsx` renders **no** `run_metadata` at all (grep: zero occurrences), so none reaches the researcher. CLAUDE.md's frontend convention is explicit: *"Trust levels rendered visibly — never silently suppress `withheld` or `degraded`"*, and guardrail #3 requires surfacing the level rather than the value. Today a researcher can be shown a replayed NAV with a degraded cash anchor, a withheld return day and flat-anchored holdings with no indication any of it happened. Precedent for the presentation exists on the Exposure side (`DriftBenchmarkPanel.tsx` renders the FX-fallback and statement-anchored notes). US-24.10 deliberately did NOT ship a lone note for its own tier — that would have made the newest, mildest disclosure the only visible one while the more severe siblings stayed hidden. Scope: surface all five together on the Dashboard, reusing the existing helper-note pattern (no hand-rolled badges — the design-system audit enforces this). |
