...
| US-24.10 — unpriced-symbol cash events fabricate investor-performance returns | High — **NEW (logged by US-24.9, 2026-08-09)** | `unpriced_replay_symbols` (BTEC / IUFS / IUHC on IB2026) contribute **$0** to `total_market_value` but their trades move **real cash**, so `total_portfolio_value` steps by the full trade amount with no offsetting position value — and the cash-inclusive TWR publishes that step as performance. Reproduced against the frozen golden data: **2026-04-08 → −7.90%** (buying $5,092.82 of BTEC/IUFS/IUHC) and **2026-04-27 → +9.61%** (selling IUFS/IUHC for $5,341.92). Nothing was earned or lost on either day. Blast radius is the **investor-performance** family only — the Dashboard performance chart, TWR, money-weighted return, monthly returns and the dashboard max drawdown; US-24.9 moved the risk statistics onto a basis that is structurally immune (it works purely in priced-market-value space), which is how the defect was found. Same guardrail-#3 class as Epic 31 F-3 (never publish a non-market move as a return): the fix is to price those symbols or to WITHHOLD the affected days with a stated reason — never to leave a fabricated return published. Evidence + measurement in `docs/product/stories/US-24.9-ledger-replay-cash-de-dilution.md` → Notes → "What the AC9 measurement found".
