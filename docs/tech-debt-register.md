...
| US-24.5 — decouple broker format from domain | Med | `domain/ledger.py:63-92` hardcoded statement section labels; entry-type pseudo-enum (`BUY/SELL/...`) lacks a shared `Literal`/`Enum`. **Validity re-check 2026-07-08: still valid and strengthened** — since US-28.1 a third importer (`interactive_brokers_csv.py`) must match these exact section strings by convention. |
...
| US-24.7 — minor hardcodes + de-export + test smells | Low | **Validity re-check 2026-07-08 — two items stale:** the `reconciliation.py` EURUSD hardcode was resolved incidentally by US-28.1 (per-currency `fx_rates` lookup; only the documented EUR legacy default remains), and the de-export motivation is obsolete (US-23.8 shipped with `ignoreExportsUsedInFile: true`, so over-exports never trip the gate — pure hygiene now). Still open: `reconciliation.py:78` inline 0.25 tolerance, `statement_importer.py` TWR `/100`, `portfolio_proof.py:1264` tolerance default, `"USD"` default repetition, hand-rolled test-builder duplication; `build_position_risk_contributions` gained one direct test in Epic 27 (gap partially addressed). |
| US-24.9 — cash de-dilution on the imported ledger-replay return series | Med — **UNBLOCKED 2026-08-04** | **UNBLOCKED 2026-08-04 — Epic 31 closed (F-1..F-5 all resolved), so the replayed series is now trustworthy and this refinement can be built on it. Original block, retained for context — BLOCKED by Epic 31 (US-31.1, 2026-07-18):** the imported ledger-replay series this row would refine is itself corrupted — price history is fetched for current holdings only while the replay reconstructs opening positions, the `starting_nav − opening_value` cash anchor absorbs the resulting $35,534 error as a plug, and the terminal reconciliation publishes that correction as a **−36.34% single-day return**, inflating annualised volatility **+79%** (Epic 31 F-1..F-3). Refining the basis on a corrupted series is meaningless and would regenerate goldens around a fabricated number — **do Epic 31 first.** **Logged by US-30.5c (Epic 30, PRD F-10); renumbered 24.8 → 24.9 by US-24.6 — the original number collided with the already-shipped `US-24.8-harden-ibkr-espp-importer-parsing` story.** US-30.5c moved the *synthetic* portfolio-return surfaces (rolling correlation/beta, factor attribution, factor model, stress, multi-benchmark) onto the cash-excluded market-value chain, but the **imported ledger-replay** path (`historical_basis="imported_portfolio_history"`) legitimately stays on the cash-inclusive TWR because a plain market-value chain there fabricates a return from any BUY/SELL (the audit F-1 class — 37.23% on the IB2026 2026-06-19 trade day). To also exclude cash on the ledger path, `DailyPortfolioState` (`schemas/reconciliation.py`) needs a per-day net **trade-flow** field recorded by `PortfolioStateEngine`, enabling a trade-neutral market-value return `r_t = (MV_t − trade_flow_t)/MV_{t−1} − 1`. Effort: schema + engine + methodology. Impact today is small (cash ≈ 3% of NAV, handled correctly by TWR), so this is an accuracy refinement, not a bug. |
