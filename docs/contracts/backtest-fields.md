...
- for the shipped desktop ranking-to-construction bridge, policy identity is now sourced from backend `GET /construction/policies` discovery rather than a silent desktop fallback; the bridge remains limited to persisted ETF ranking and persisted intent-bound ETF replacement ranking artifacts, desktop launch pickers surface only launch-compatible `top_n_equal_weight_v1` and explicit opt-in `top_n_linear_rank_weight_v1`, equal-weight remains the only auto-selected default when discovered, desktop launch must pass an explicitly selected discovered `policy_id`, desktop must fail closed if discovery does not truthfully advertise `launch_top_n = 2`, persisted construction run lineage must echo both requested `policy_id`, non-empty `policy_definition_id`, and `normalized_inputs.top_n = 2`, decimal `max_position_weight` remains required, optional decimal `min_position_weight` may be supplied or omitted, and optional decimal `max_turnover_weight` plus optional integer `max_trade_intent_count` may now also be supplied or omitted from the same review-only bridge without widening ranking-family or policy scope
