...
- `max_sector_weight` (Epic 3 breadth, 2026-05-12) is an optional decimal hard constraint capping the summed target weight of any single sector across selected names. Backend-only in this slice: the construction schema, engine evaluation (`constraint_evaluations[].constraint_id = "max_sector_weight"`), policy-catalog `max_sector_weight_constraint` capability metadata, and `ConstructionRankedCandidateInput.sector` / `ConstructionSelectedName.sector` carriers all exist; the desktop construction browsers do not yet send `max_sector_weight` or render the sector evaluation (deferred to a later slice). The request invariant `max_sector_weight >= max_position_weight` is enforced. When any selected name lacks a `sector` label the constraint evaluation is `not_evaluated`
