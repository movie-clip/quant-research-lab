...
- the echoed `replay_provenance.hard_constraints` shape is exact and fail-closed: `full_investment`, `long_only`, `eligible_ranked_universe_only`, `max_position_weight`, `min_position_weight`, `max_turnover_weight`, `max_trade_intent_count`, and `max_sector_weight`; nullable fields stay explicit `null` when the persisted artifact carries no value
- `max_sector_weight` (Epic 3 breadth, 2026-05-12) is an optional decimal hard constraint capping the summed target weight of any single sector across selected names. The construction schema, engine evaluation (`constraint_evaluations[].constraint_id = "max_sector_weight"`), policy-catalog `max_sector_weight_constraint` capability metadata, and `ConstructionRankedCandidateInput.sector` / `ConstructionSelectedName.sector` carriers all exist. The request invariant `max_sector_weight >= max_position_weight` is enforced. When any selected name lacks a `sector` label the constraint evaluation is `not_evaluated`
- sector metadata source (milestone slice 2, 2026-05-12): `generic_ranking` artifact handoffs carry per-symbol sector — `UniverseSpecSnapshot.member_sectors` (`symbol -> GICS sector`) is populated by the universe resolver for screen / `index_constituent` universes, stamped onto `GenericRankingRow.sector`, and threaded into the construction ranked-candidate contract. `etf_peer_group` / `custom_list` universes and the `etf_ranking` / `intent_bound_etf_replacement_ranking` handoffs carry no sector, so those runs evaluate `max_sector_weight` as `not_evaluated`
- desktop input (milestone slice 3, 2026-05-12): all three Workspace construction browsers (`PersistedEtfRankingConstructionBrowser`, `PersistedReplacementRankingBrowser`, `PersistedGenericRankingConstructionBrowser`) expose an optional `Max Sector Weight` input. Shared validation (`validateRankingConstructionConstraintInputs` / `validateRankingConstructionMaxSectorWeightInput`) enforces a decimal in `(0, 1]` and the `max_sector_weight >= max_position_weight` invariant client-side; blank omits the field from the request. `runRankingArtifactConstructionHandoff` threads it into `hard_constraints.max_sector_weight`. The same slice also fixed a latent bug where the handoff runner hard-rejected `top_n != 2`, which had silently disabled the configurable-`top_n` path end-to-end; it now accepts `top_n` in `[2, 20]` and checks run lineage against the requested value
