...
- for the shipped desktop ranking-to-construction bridge, policy identity is now sourced from backend `GET /construction/policies` discovery rather than a silent desktop fallback; the bridge remains limited to persisted ETF ranking and persisted intent-bound ETF replacement ranking artifacts, desktop launch must pass an explicitly selected discovered `policy_id`, desktop must fail closed if discovery does not truthfully advertise `launch_top_n = 2`, persisted construction run lineage must echo both requested `policy_id`, non-empty `policy_definition_id`, and `normalized_inputs.top_n = 2`, decimal `max_position_weight` remains required, optional decimal `min_position_weight` may be supplied or omitted, and other optional constraints remain hidden in this review-only bridge
