...
| UI field | Current UI/provider source | App state source | Truth class | Unavailable rule | Notes |
| --- | --- | --- | --- | --- | --- |
| Indexed chart (portfolio vs benchmark) | `PerformanceBenchmarkCard.tsx` `buildIndexedSeries(result.performance_series, rangeMetrics[activeRange].window_start_date ?? null)` | `analysis.performance_series`, `analysis.range_metrics[activeRange].window_start_date` | `engine-derived` | if no non-null series points, render `EmptyState` | base-100 rebasing per §Indexed Return Series; portfolio leg re-based off `portfolio_return_pct` (2026-08-26 chart-audit fix — was previously a raw `portfolio_value` ratio, a fabricated line that overstated returns ~10x on a fixture with a mid-period deposit); filtered to `>= window_start_date` so the chart re-anchors with the range selector instead of always plotting full history regardless of `activeRange` (same audit) |
...
| UI field | Current UI/provider source | App state source | Truth class | Unavailable rule | Notes |
| --- | --- | --- | --- | --- | --- |
| Portfolio / Benchmark / Downside Volatility, Tracking Error | `RiskSummaryCard.tsx` from `diagnosticsAnalysis.volatility_summary` | `diagnosticsAnalysis` (separate App.tsx state, not `analysis`) | `engine-derived`, synthetic-history basis | `n/a` per null field; whole-card `EmptyState` if `diagnosticsAnalysis` or any of its three summary sub-objects is absent | §Volatility and Relative Risk. Portfolio Volatility here has **no** minimum-observation floor — it publishes from N ≥ 2 (`null` at N = 1, matching `portfolio_beta`/`portfolio_correlation`/`r_squared` in the same struct; fixed from a prior `0.00%`-at-N=1 bug). US-44.1 added a 60-paired-observation floor to the *Risk-tab* view of this same scalar (`risk_tab_volatility`, see `risk-fields.md` §US-44.1); this Dashboard row is unchanged. |
| Current / Max Drawdown | `diagnosticsAnalysis.drawdown_summary` | `diagnosticsAnalysis` | `engine-derived`, synthetic-history basis | same as above | **Deliberately not** `DashboardHistoryResult.max_drawdown_pct` (that field stays withheld under the investor-economics policy below); diagnostics' drawdown is a separate, unwithheld path — publishes whenever `historical_sections_available` is `True` (the only call site), with the remaining math-layer null cases unchanged; no categorical `withheld` gate remains on this field — see US-25.3 story Context and `risk-fields.md:548`'s "no withheld rung" policy for the Risk-tab drawdown surface's matching convention |
...
| Sector composition donut | `SectorPieCard.tsx` | `result`, `exposureResult` | `engine-derived` | replaced the earlier editable Allocation Overview draft (no longer present anywhere in the codebase); as of the sector/benchmark card merge it renders as a `role="group"` sub-section, side by side with Benchmark Positioning, not its own top-level card; its visibility is governed by the shared card's single fold toggle (see merge note below), not an independent one |
| Benchmark Positioning | `BenchmarkPositioningCard.tsx` | `exposureResult` | `engine-derived` | as of the sector/benchmark card merge it renders as a `role="group"` sub-section, side by side with Sector Composition, sharing one top-level `summary-card` instead of its own card; it is **not** independently foldable — its own fold state, toggle button, and `aria-controls` wrapper were removed, and visibility is governed by the shared card's single fold toggle (see merge note below) |

**Sector/Benchmark card merge:** `DashboardPanel.tsx` wraps both sub-components in one `<section className="summary-card dashboard-composition-card" aria-label="Sector and Benchmark Composition">` with a divider between them. Each sub-component's outer element changed from its own `<section className="summary-card">` to a `<div role="group" aria-label="…">`, so each is queryable via `getByRole('group', { name: ... })`. The two sub-sections render **side by side** in a `.dashboard-composition-row` grid (`1fr auto 1fr`, vertical divider) rather than stacked, collapsing to a single column with a horizontal divider under a `max-width: 1100px` breakpoint. Folding is owned entirely by `DashboardPanel.tsx` via one `compositionExpanded` state and a single header/toggle button for the shared card; both sub-sections' content renders (or is hidden) together, not independently — `BenchmarkPositioningCard.tsx` has no fold state or toggle of its own. This is a presentational/DOM-shape change only — no field, formula, or trust class changed on either card.
...
- `range_metrics[*].summary.money_weighted_return_pct` / `investment_gain` (US-34.6 / Epic 34 F-7): both are **performance** figures and are computed from the **market-derived terminal value** — the terminal `total_portfolio_value` less its `reconciliation_adjustment` — so neither republishes the statement reconciliation that US-31.3 withholds from the time-weighted return. On IB2026 (2026-08-17 capture) the money-weighted return is 2.76% against 2.73% with the entry left in, and the gain is $1,645.99 against $1,626.01 — the entry is worth 0.03pp and $19.98. It was far larger when US-34.6 shipped (2.35pp and $1,366.17); US-34.3 and US-34.9 since shrank the terminal reconciliation to −$19.98, which changes the size of the correction but not the rule. **`end_value`, `start_value`, `net_contributions` and every daily state keep the reconciled value** — the statement's ending NAV is broker truth and is the correct *level*. The consequence is that `end_value − start_value − net_contributions ≠ investment_gain` on a reconciled run, which the Dashboard must explain on screen rather than leave looking like an arithmetic error.
...
- `range_metrics[*].window_start_date` (CR-2 #1, 2026-08-26 Performance & Benchmark chart audit): `string | null` on the backend (`DashboardRangeMetrics.window_start_date`, `app/schemas/dashboard_history.py`) — the ISO date the range's own window begins, `null` only for `All`. Populated straight off the same slice `_slice_performance_series` (`dashboard_history_engine.py`) already computes for that range's summary scalars — no new computation, just publishing a date that existed internally and was not exposed. **YTD is not null** even though it shares `window=None` with `All` in `RANGE_WINDOWS`: it is handled by its own year-filter branch and resolves to the year's first trading day (verified in `10-quant-audit.md` §Log/3). **Exists to fix a real bug, not just to add a field:** before this, `PerformanceBenchmarkCard.tsx`'s chart always plotted the full imported `performance_series` regardless of the range selector, so choosing "1M" changed every number in the summary strip beneath the chart but left the chart itself showing years of history — contradicting US-25.1 AC7's promise that the selector "re-anchors the chart" (see `cr/CR-2.md` CHANGE REQUEST 1 in the audit run). `buildIndexedSeries` now filters `performance_series` to `>= window_start_date` and re-bases the sliced portfolio leg's already-published `portfolio_return_pct` chain to 100 at that point (`null` → unfiltered, the original day-one-anchored "All" behavior, unchanged). The re-basing formula itself is unchanged — algebra on an already-correct TWR chain per §Indexed Return Series, no new formula. TS mirror: `types.ts` `DashboardRangeMetrics.window_start_date?: string | null` — typed optional where the backend always serializes the field (judged safe-direction, non-blocking: `PerformanceBenchmarkCard.tsx` normalizes via `?? null` before use, and the sibling `portfolio_return_trust?:` field is the same established shape in this type); see `docs/tech-debt-register.md` for the open tightening item.
