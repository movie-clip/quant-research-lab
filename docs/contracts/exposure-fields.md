...
3. `bootstrap: ImportedBootstrapResponse` (backend: `services/quant-engine/app/schemas/import_bootstrap.py`)
   - the analyze-upload import response — a second root source alongside `ExposureResult` (`runExposureEngine`'s response)
   - built by `build_import_bootstrap_from_snapshot(...)` (`services/quant-engine/app/services/import_engine.py`), which calls the same `build_exposure_result(...)` (`services/quant-engine/app/services/exposure_engine.py`) `runExposureEngine`'s route uses, then forwards fields onto `ImportedBootstrapResponse` via `_compose_import_bootstrap_response(...)`, a module-private helper inside `services/quant-engine/app/services/import_engine.py` (folded in from the now-deleted `import_engine_composer.py` in US-43.4)
   - carries `overview`, `lookthrough`, `lookthrough_sector_exposure`, `market_overlap`, `current_state_concentration`, `availability` — the same 5 exposure fields this document tracks for `ExposureResult`, plus `overview`
   - mirrored on the frontend by `ImportedBootstrapResponse` / `ImportedPortfolioSnapshotSource` (`apps/desktop/src/features/portfolio/types.ts`) and mapped into workspace state by `projectImportedBootstrap` (`apps/desktop/src/features/portfolio/importedBootstrapMapper.ts`)
   - **distinct truth class from `ExposureResult`'s own path**: these 6 fields are computed once, at import time, and — as of the 2026-08-24 bug-fix slice — persisted and replayed by the frontend's persist-and-reuse mechanism (`ImportedExposureOverride`, `apps/desktop/src/app/portfolioWorkspaceStorage.ts`) on session restore and snapshot switching, rather than recomputed on every render; they are frozen-at-import values, not live/per-render values the way `ExposureResult`'s own fields are when sourced straight from `runExposureEngine(...)`
...
- `currency_exposure` (US-26.1): per-currency composition of the portfolio — `weights[]` (`currency` / `market_value` / `weight`, sorted by weight descending), `non_base_weight`, `total_base_market_value` and `base_currency`. **Weights are computed on base-currency-converted market values in both numerator and denominator**, then grouped by the currency each position is *denominated in* — group by original currency, measure in one unit. The denominator is the same `total_base_market_value` every other Exposure weight uses (US-30.5a), so this card can never disagree with the concentration or sector cards on the same screen; a regression test pins that equality. `non_base_weight` is **null** when the statement carries no base currency (no baseline to compare against; `0` would read as "no currency risk") and renders as "—". Currencies carried **unconverted** because no FX rate exists are still counted in the denominator and disclosed through the existing `fx_fallback_currencies` tier — such a currency's own weight is the least trustworthy number on the card. Snapshot analytics: current holdings only, **no market-data call** (structurally pinned). See `financial-methodology.md` §Currency Exposure.

  *Note on the research brief's "unclassified" bucket:* it is unreachable — `ImportedPosition.currency` is `str = Field(min_length=3, max_length=3)`, so a currency-less position cannot exist in a snapshot. The fabrication the brief anticipated is real but happens upstream, in the request-path snapshot builder (`portfolio_snapshot_builder.py`), which coerces a missing currency to the base currency before any analytic sees it. Recorded as its own tech-debt row rather than hidden behind a bucket that can never fill.

- `CurrencyRiskResult` (US-26.2, `POST /engines/currency-risk/run`): the local / currency / interaction variance split for a 60d or 252d window. **Truth class is synthetic history** (current holdings × historical prices) — `trust` is `'synthetic' | 'unavailable'`, never `verified`, and the card carries a `Synthetic` badge (unlike US-26.1's composition card, which is snapshot analytics). The three shares are **component covariances** — `Cov(L, r_p)/Var(r_p)` etc. — so they sum to **exactly 1.0** by construction; the naive `Var(F)/Var(r_p)` is deliberately not used because it ignores `Cov(L, F)` and does not account for the portfolio. **A share may be negative** and is never clamped: a currency leg moving against the local leg genuinely reduces portfolio variance, and a floor would fabricate confidence. Local returns are computed in each holding's **registry fund currency** (US-31.5), never the broker's listing currency — the two differ (DEFS is listed EUR but `DEFS.L` quotes USD), and a wrong assignment silently moves return between the legs. The **interaction leg is reported as its own share**, not folded into currency as Ankrim–Hensel conventionally does: it is negligible per day (≤5.4bp measured) but compounds to +0.504pp of cumulative return over six months on IB2026's SEMI. Fail-closed: fewer than `MIN_DAILY_OBSERVATIONS` (20) paired price+FX days, or `Var(r_p) = 0`, nulls every share (never 0 or 1). A holding with no fund-currency price history is **excluded and named** in `excluded_symbols` with its `excluded_weight` reported — never assigned to the local leg at zero FX. See `financial-methodology.md` §Currency Risk Contribution for the formulas and `docs/contracts/currency-risk-fields.md` for the field-level inventory (`CurrencyRiskRequest` / `CurrencyRiskResult` / `CurrencyLegContribution`).
